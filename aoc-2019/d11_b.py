...
curr = (0, 0)
paint = {}
output, i, base, done = intcode(program, i = 0, base = 0, input = [1])
while not done:
    color, dir = output
    assert color in [0, 1]
    assert dir in [0, 1]
...
min_x = min(paint, key=lambda x: x[0])[0]
min_y = min(paint, key=lambda x: x[1])[1]
max_x = max(paint, key=lambda x: x[0])[0]
max_y = max(paint, key=lambda x: x[1])[1]
m = {0 : " ", 1 : "#"}
